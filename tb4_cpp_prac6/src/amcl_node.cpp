...
namespace iar_amcl
{
  ...
  AmclNode::pf_resample(pf_t *pf)
  {
    ...
    */
    ...
    {
      cd_old_set[i + 1] = cd_old_set[i] + old_particle_set->samples[i].weight;
    }
    ...
    */
    double w_diff = 1 - (pf->w_fast / pf->w_slow);
    ...
    while (new_particle_set->sample_count < pf->max_samples)
    {
      ...
      if (drand48() < w_diff)
      {
        ...
        */
        pf_vector_t random_poses = (pf->random_pose_fn)(reinterpret_cast<void *>(map_));
        sample_in_new_set->pose.v[0] = random_poses.v[0];
        sample_in_new_set->pose.v[1] = random_poses.v[1];
        sample_in_new_set->pose.v[2] = random_poses.v[2];
      }
      else
      {
    ...
    */
    ...
    {
      new_particle_set->samples[i].weight /= total_weight;
    }
